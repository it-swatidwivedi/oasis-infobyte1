...
def BMI():
    ...
    if bmi<=18.5:
        ...
        label3.config(text="it indicate that u are healthy!")

    elif bmi>25 and bmi<=30:
        label2.config(text="Overweight!")
        label3.config(text="it indicate a person is \n slightly overweight \n A doctor may advise to lose some \n weight for health reason!")

    else:
        label2.config(text="Obesity!")
        label3.config(text="Health may be at risk,if they do not consult to doctor!")
...
style = ttk.Style()
style.configure("Tscale",backgroung="white")
...
style2 = ttk.Style()
style2.configure("Tscale",backgroung="white")
...
Height=StringVar()
Weight=StringVar()
height=Entry(root,textvariable=Height,width=5,font='arial 50',bg="#fff",bd=0,justify=CENTER)
height.place(x=35,y=160)
Height.set(get_current_value())

weight=Entry(root,textvariable=Weight,width=5,font='arial 50',bg="#fff",bd=0,justify=CENTER)
